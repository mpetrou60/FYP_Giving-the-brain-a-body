...
int motion = 0;     // forward, backward, coiling behaviours
int n = 0;
...
void setup() {
  ...
  // make sure all muscles are set to zero (like a second reset)
...
void loop() {
  // Reset motion counter to repeat sequence
  if (motion >= 31) {
      motion = 0;
    }
  ...
  // update oscillation counter (motion counter increments for every complete oscillation of stimulation neurons)  
  if (n == 18) {
      n = 0;  
      motion += 1;
  }
